...
//@WebMvcTest -> This provides in-memory userService to auth trying to minimize the things to up
@SpringBootTest
class DatabaseAuthenticationApplicationTests {
    ...
    fun simpleBasicHttpNoAuthURL() {
        mockMvc.perform(
            MockMvcRequestBuilders.get("/no-auth/")
    ...
    fun simpleBasicHttpAPIGet() {
        mockMvc.perform(
            MockMvcRequestBuilders.get("/api/something/banana")
                .with(SecurityMockMvcRequestPostProcessors.anonymous())
        )
            .andExpect(MockMvcResultMatchers.status().isOk)
    }
    ...
    fun simpleBasicHttpAPIPost() {
        mockMvc.perform(
            MockMvcRequestBuilders.post("/api/something/banana")
                .with(SecurityMockMvcRequestPostProcessors.anonymous())
        )
    ...
    fun simpleBasicHttpAPIAnythingGet() {
        mockMvc.perform(
            MockMvcRequestBuilders.get("/api/anything/banana")
                .with(SecurityMockMvcRequestPostProcessors.anonymous())
        )
            .andExpect(MockMvcResultMatchers.status().isOk)
    }
