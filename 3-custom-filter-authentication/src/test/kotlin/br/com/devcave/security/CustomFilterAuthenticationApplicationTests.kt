...
@WebMvcTest
class CustomFilterAuthenticationApplicationTests {
	...
	/**
	 * Here we are testing passing user and password on headers
	 * */
	@Test
	fun passwordHeaderLogin() {
		mockMvc.perform(MockMvcRequestBuilders.post("/api/something/someting")
	...
	/**
	 * Here we are testing passing wrong user and password on headers
	 * */
	@Test
	fun passwordHeaderWrongCredentials() {
		mockMvc.perform(MockMvcRequestBuilders.post("/api/something/someting")
			.header("Api-Key", "scottXXX")
			.header("Api-Secret", "tigerXXX"))
			.andExpect(MockMvcResultMatchers.status().isUnauthorized)
	}
}
